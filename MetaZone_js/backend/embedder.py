...
class EmbedSession:
    ...
    def dry_run(self, folder, columns, options):
        """v0.9.4 (item 14): categorizes every CSV row against the same
        one-time file index preview_match/the real run use, without
        touching a single file on disk. Categories this can honestly
        distinguish with the existing matching logic:
          - matched: filename present, a file was found for it, and
            that file's extension is one the app actually embeds into.
        ...
        "Already embedded" is deliberately NOT a category here -- that
        would need reading EXIF back out of every matched file (a real
        exiftool subprocess call per file), a materially heavier
        operation than this index lookup. Flagged as a known gap
        rather than faked with a placeholder count.
        """
        ...
        rows_out = []
        counts = {"matched": 0, "missing_image": 0, "missing_metadata": 0, "unsupported": 0}
        for i, row in enumerate(self.csv_rows):
            ...
            counts[category] += 1
            rows_out.append({"row": i, "filename": fn, "category": category})
