...
class Session:
    ...
    def restore_card(self, path, result):
        """v0.9.4: Undo for delete_card/delete_cards (item 4 of the UX
        spec). Re-adds the path to all_paths/results/completion_order
        and rewrites the working CSV if one exists, mirroring
        delete_card's own bookkeeping in reverse. Honest limitation:
        this appends the path back rather than reinserting at its
        exact original backend-side index -- the frontend restores the
        card's exact visual position itself (it keeps the actual DOM
        node), so what the person sees is exact; only this session's
        own internal ordering (which only matters for a fresh CSV
        export or future remaining-count math) is append-only."""
