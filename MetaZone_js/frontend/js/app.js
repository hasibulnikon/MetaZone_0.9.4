...
function updateSelectionUI() {
  ...
  document.getElementById('btnGenerateSelected').textContent = `✨ Generate Selected (${count})`;
...
BackendEvents.on('cards_removed', (p) => {
  ...
  if (snaps.length) {
    Toast.show(`${snaps.length} card${snaps.length === 1 ? '' : 's'} removed`, {
      type: 'info', duration: 7000,
      action: { label: 'Undo', onClick: () => snaps.forEach(([path, snap]) => restoreCard(path, snap)) },
    });
  }
});
